...
- **工作区工具**：桌面端可直接读取系统账户有权访问的任意目录；工作区内可创建、写入、移动和删除文件，工作区外变更按完全访问设置直接执行或逐次审批；同时支持运行 npm 脚本、安全白名单命令以及查看 Git 状态和差异。
...
- Electron 桌面版的读取工具可访问当前系统账户有权读取的任意目录；关闭“运行完全访问”时，工作区外的创建、写入、编辑、移动和删除会逐次请求授权。命令执行和 Git 操作仍可能改变文件或仓库状态，请在授权前确认操作内容。
