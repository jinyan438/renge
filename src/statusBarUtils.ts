...
export function resolveStatusBarContextMacros(value: string, userName: string) {
  const resolvedUserName = userName.trim() || "用户";
  return value.replace(/\{\{\s*user\s*\}\}/gi, () => resolvedUserName);
}
    ...
    character.items
      ...
      .map((item) => {
        ...
        return {
          ...
          ...(promptMetadata.variableName !== item.variableName
            ? { sourceVariableName: item.variableName }
            : {}),
  ...
  const entries = getStatusBarEntriesForPrompt(state).map((entry) => {
    ...
    return {
      ...
      ...(entry.sourceVariableName
        ? { sourceVariableName: entry.sourceVariableName }
        : {}),
      description: entry.description,
  ...
  const fields = getStatusBarEntriesForPrompt(state)
    ...
    .map((entry) => {
      ...
      return {
        ...
        characterKind: entry.characterKind,
        name: entry.variableName,
        ...(entry.sourceVariableName ? { sourceName: entry.sourceVariableName } : {}),
