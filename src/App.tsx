...
const htmlPreviewBootstrapScript = [
  ...
  "    const parsed = new DOMParser().parseFromString(isolateRemoteParentReferences(html), \"text/html\");",
  ...
  "    window.__rengeInstallLegacySendControls?.();",
  ...
  "    };",
  "  };",
  "  if (existingDollar?.fn) {",
  "    if (!existingDollar.__rengeParentIsolationInstalled) {",
  "      const originalHtmlPrefilter = typeof existingDollar.htmlPrefilter === \"function\"",
  "        ? existingDollar.htmlPrefilter.bind(existingDollar)",
  "        : (html) => String(html || \"\");",
  "      existingDollar.htmlPrefilter = (html) =>",
  "        originalHtmlPrefilter(isolateRemoteParentReferences(html));",
  "      const originalLoad = existingDollar.fn.load;",
  "      if (typeof originalLoad === \"function\") {",
  "        existingDollar.fn.load = function(resource, data, complete) {",
  "          if (typeof data === \"function\") { complete = data; data = undefined; }",
  "          const wrappedComplete = function(...args) {",
  "            window.__rengeInstallLegacySendControls?.();",
  "            if (typeof complete === \"function\") return complete.apply(this, args);",
  "          };",
  "          return originalLoad.call(this, resource, data, wrappedComplete);",
  "        };",
  "      }",
  "      existingDollar.__rengeParentIsolationInstalled = true;",
  "    }",
  "    return;",
  "  }",
...
function buildHtmlPreviewVariablesScript(previewId: string, context: HtmlPreviewContext) {
  ...
  return [
    ...
    "    value: (onFulfilled, onRejected) => Promise.resolve()",
    "      .then(() => typeof onFulfilled === \"function\" ? onFulfilled({ ...context }) : { ...context })",
    "      .catch(onRejected),",
    "  });",
    "  return context;",
    "};",
    ...
    "const installLegacySendControls = () => {",
    "  const root = document.body || document.documentElement;",
    "  if (!root) return;",
    "  let textarea = document.getElementById(\"send_textarea\");",
    "  if (!textarea) {",
    "    textarea = document.createElement(\"textarea\");",
    "    textarea.id = \"send_textarea\"; textarea.hidden = true; root.appendChild(textarea);",
    "  }",
    "  const syncLegacyInput = () => { void setInput(String(textarea.value ?? \"\")); };",
    "  if (typeof window.jQuery === \"function\") {",
    "    window.jQuery(textarea)",
    "      .off(\".rengeLegacyInputBridge\")",
    "      .on(\"input.rengeLegacyInputBridge change.rengeLegacyInputBridge\", syncLegacyInput);",
    "  } else if (!textarea.dataset.rengeInputBridge) {",
    "    textarea.addEventListener(\"input\", syncLegacyInput);",
    "    textarea.addEventListener(\"change\", syncLegacyInput);",
    "    textarea.dataset.rengeInputBridge = \"true\";",
    "  }",
    "  let sendButton = document.getElementById(\"send_but\");",
    "  if (!sendButton) {",
    "    sendButton = document.createElement(\"button\");",
    "    sendButton.id = \"send_but\"; sendButton.type = \"button\"; sendButton.hidden = true;",
    "    root.appendChild(sendButton);",
    "  }",
    "  const submitLegacyInput = () => { void sendMessage(String(textarea.value ?? \"\")); };",
    "  if (typeof window.jQuery === \"function\") {",
    "    window.jQuery(sendButton)",
    "      .off(\".rengeLegacySendBridge\")",
    "      .on(\"click.rengeLegacySendBridge\", submitLegacyInput);",
    "  } else if (!sendButton.dataset.rengeSendBridge) {",
    "    sendButton.addEventListener(\"click\", submitLegacyInput);",
    "    sendButton.dataset.rengeSendBridge = \"true\";",
    "  }",
    "};",
    "window.__rengeInstallLegacySendControls = installLegacySendControls;",
    "if (document.body) installLegacySendControls();",
    "else document.addEventListener(\"DOMContentLoaded\", installLegacySendControls, { once: true });",
...
function buildHtmlPreviewScript(previewId: string, heavyContent: boolean) {
  ...
  return [
    ...
    "};",
    "const getExpandedDetailsHeight = () => {",
    ...
    "    const expandedHeight = Math.max(rect.height || 0, element.scrollHeight || 0, element.offsetHeight || 0);",
    "    minTop = Math.min(minTop, top);",
    "    maxBottom = Math.max(maxBottom, top + expandedHeight, rect.bottom + scrollY);",
    "  });",
    "  return Math.max(0, maxBottom - Math.min(0, minTop));",
    "};",
    ...
    "  const usesProvisionalViewport = viewportHeight <= defaultHeight + 64;",
    "  if (!contentHeight || (!bounds.hasIntrinsicElement && usesProvisionalViewport && (tracksViewport || tracksRecentViewport))) {",
    ...
    "  const shouldForcePost = forceNextPost;",
    "  forceNextPost = false;",
    "  if (!shouldForcePost && Math.abs(height - lastHeight) < 1 && Math.abs(reportedExpandedHeight - lastExpandedDetailsHeight) < 1 && hasExpandedDetails === lastExpandedDetailsState) return;",
    "  lastHeight = height;",
    ...
    () =>
      visibleChatMessages.map((message, index) => {
        ...
        const regexOptions = {
          ...
          depth,
          userName: userProfile.nickname,
          messageVariables: isObjectRecord(message.variables) ? message.variables : {},
          characterName:
