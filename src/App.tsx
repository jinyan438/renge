...
function splitEmbeddedHtmlBlocks(content: string): PlainChatHtmlSegment[] {
  ...
  const htmlStartPattern =
    /(^|\n)([ \t]*)(<!doctype\s+html\b|<([A-Za-z][\w:-]*)(?=[\s>/]))/gi;
  let cursor = 0;
  let searchCursor = 0;
...
function buildHtmlPreviewScript(previewId: string) {
  ...
  return [
    ...
    "} catch {}",
    "document.addEventListener(\"DOMContentLoaded\", schedulePost, { once: true });",
    "schedulePost();",
    "[80, 240, 600, 1200, 2400, 4000].forEach((delay) => setTimeout(schedulePost, delay));",
